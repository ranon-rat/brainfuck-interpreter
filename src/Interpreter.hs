...
interpreterWithMoreStuff instructions pos cells currentCell =do 
    if pos>=length instructions then return (cells,currentCell)
        ...
        if cells!!currentCell<0 then do 
            ...
            interpreterWithMoreStuff instructions (pos)  (x++[255]++ys) currentCell
        else if cells!!currentCell>255 then do
            let (x,_:ys) =splitAt currentCell cells
            interpreterWithMoreStuff instructions (pos)  (x++[0]++ys) currentCell
        else do 
            case  instructions!!pos of
                Move a-> interpreterWithMoreStuff instructions (pos+1)  (if (currentCell+a)<length cells then  cells else  cells++[0]) (abs (currentCell+a))
                Change a->do 
                    let (x,_:ys) =splitAt currentCell cells
